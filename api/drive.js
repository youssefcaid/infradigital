...
// إنشاء JWT للحصول على Access Token
function createJWT() {
...
// قراءة Folder بالكامل بشكل recursive
...
function findCategory(module, names) {
  ...
  return module.children.find(item =>
    item.type === "folder" &&
    target.includes(item.name.trim().toLowerCase())
  );
}
    ...
    for (const [yearName, rootId] of Object.entries(ROOTS)) {
      ...
      const modules = rootItems
        ...
        .map(module => {
          ...
          const cours = findCategory(module, [
            "cours",
            "course",
            "cours"
          ]);

          const exercices = findCategory(module, [
            "exercices",
            "exercice"
          ]);

          const controle = findCategory(module, [
            "contrôle",
            "controle",
            "contrôles",
            "controles"
          ...
          return {
            ...
            exercices: exercices
              ? countPDFs(exercices.children)
              : 0,

            controle: controle
              ? countPDFs(controle.children)
              : 0,

            efm: efm
              ? countPDFs(efm.children)
              : 0,
